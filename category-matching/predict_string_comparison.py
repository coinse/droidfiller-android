...
if __name__ == "__main__":
    ...
    argparser.add_argument("-o", "--output-path", help="Path to save the result files", default="result/text_input_pred.json")
    ...
    samsung_internal_states = glob.glob(os.path.join('data/samsung_internal', '**', 'state_*.json'), recursive=True)
    OSS_states = glob.glob(os.path.join('data/OSS/converted', '**', 'state_*.json'), recursive=True)
    combined_states = samsung_internal_states + OSS_states

    for state_path in tqdm(combined_states):
        screen_path = state_path.replace('state', 'screen').replace('.json', '.png')
        tfs = load_textfields_from_state(state_path, screen_path)
        if len(tfs) == 0:
            print(f'No textfields found in {state_path}')
